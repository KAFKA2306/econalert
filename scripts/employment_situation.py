...
def parse_release(raw:bytes,*,source_url:str,retrieved_at:str):
    ...
    payroll=int(pm.group(2).replace(",",""))/1000
    ...
    for groups in [(1,2,3,4),(5,6,7,8)]:
        ...
        prevk=int(prev.replace(",",""))//1000; newk=int(new.replace(",",""))//1000; amtk=int(amt.replace(",",""))//1000
    ...
    relmon=MONTHS[rid.group(2).upper()]
    payload={"schema_version":1,"publisher":"U.S. Bureau of Labor Statistics","dataset":"Employment Situation",
      "reference_month":f"{ry}-{rm:02d}","release_id":f"USDL-{rid.group(1)}",
      "release_timestamp":f"{rid.group(4)}-{relmon}-{int(rid.group(3)):02d}T08:30:00-04:00",
      "retrieved_at":retrieved_at,"source_url":source_url,"source_sha256":hashlib.sha256(raw).hexdigest(),
      "verification_state":"VERIFIED_HISTORICAL_RELEASE","series":SERIES,
      "metrics":{"payroll_change":payroll,"unemployment_rate":ur,"labor_force_participation_rate":part,"average_hourly_earnings":ahe},
      "payroll_revisions":revisions}
    validate_snapshot(payload); return payload

def validate_snapshot(p):
    required={"schema_version","publisher","dataset","reference_month","release_id","release_timestamp","retrieved_at","source_url","source_sha256","verification_state","series","metrics","payroll_revisions"}
    ...
    if not re.fullmatch(r"[0-9a-f]{64}",str(p["source_sha256"])): raise ValueError("invalid source hash")
...
def render(p):
    ...
    latest["authority"]="KAFKA2306/econalert"; latest["source"]={"url":p["source_url"],"sha256":p["source_sha256"],"release_id":p["release_id"]}
    ...
    manifest={"schema_version":1,"dataset":"Employment Situation","reference_month":p["reference_month"],"source_sha256":p["source_sha256"],"outputs":{"latest.json":hashlib.sha256(jb).hexdigest(),"latest.csv":hashlib.sha256(cb).hexdigest()}}
